...
public class Animation extends Thread {
	...
	public void run() {
		...
		parent.log("Animation execution finished");
	}
		...
		if (!fbasic) {
			...
			while (tool.hasNextArg()) {
				try {
					switch (tool.nextArg()) {
						case POINT:
							tool.input(Point.fromCenter0_0(new Point(Integer.parseInt(instruction[i++]), Integer.parseInt(instruction[i++])), App.CANVAS_COLS, App.CANVAS_ROWS));
							break;
						case VALUE:
